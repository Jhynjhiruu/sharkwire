...
parser.add_argument(
    "-w", "--which", type=str, default="loader", help="diff loader or browser"
)
...
match args.which:
    case "loader":
        ...
        myimg = f"build/sharkwire.bin"
        mymap = f"build/loader.map"
    
    case "browser":
        baseimg = f"assets/loader/browser.bin"
        basemap = f"build/browser.map"

        myimg = f"build/browser.bin"
        mymap = f"build/browser.map"
